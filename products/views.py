    ...
    if request.method == 'POST':
        try:
            ...
            PriceChangeLog.objects.create(
                ...
                new_price=new_price,
                weekly_sales=product.weekly_sales,
                demand_score=product.calculate_demand_score()
            )
...
def pricing_analytics(request):
    ...
    try:
        ...
        for product in products:
            products_data.append({
                'id': product.id,  # Add ID for tooltips
                'title': product.title,
                'selling_price': float(product.selling_price),
                'base_price': float(product.base_price),
                'max_price': float(product.max_price),
                'weekly_sales': product.weekly_sales,
                'last_week_sales': product.last_week_sales,
                'demand_high': product.demand_threshold_high,  # Fixed field name
                'demand_low': product.demand_threshold_low,    # Fixed field name
                'stock_count': product.stock_count,  # Added for future use
            })
        ...
        for change in recent_changes:
            changes_data.append({
                'product_id': change.product.id,  # Add ID
                'product_title': change.product.title,
                'old_price': float(change.old_price),
                'new_price': float(change.new_price),
                'timestamp': change.timestamp.isoformat(),
                'reason': change.reason if hasattr(change, 'reason') else 'Auto-adjustment'
            })
        ...
        context = {
            ...
            'stats': {
                ...
                'avg_margin': avg_margin  # Now calculated in view
            }
        }
        ...
        logger.error(f"Error in pricing_analytics: {str(e)}")
