...
class Product(models.Model):
    ...
    demand_threshold_high = models.IntegerField(
        default=20, verbose_name="High Demand Threshold"
    )
    demand_threshold_low = models.IntegerField(
        default=5, verbose_name="Low Demand Threshold"
    ...
    def get_predicted_price(self):
        ...
        try:
            ...
            model_path = os.path.join(settings.BASE_DIR, 'pricing_model.pkl')
...
class PriceChangeLog(models.Model):
    ...
    def __str__(self):
        def __str__(self):
            return f"{self.product.title}: ${self.old_price:.2f} → ${self.new_price:.2f} ({self.reason})"